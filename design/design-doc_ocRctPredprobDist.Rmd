...
## h_decision_one_rct_predprobDist when ` relativeDelta = FALSE`.
...
## h_decision_one_rct_predprobDist when ` relativeDelta = TRUE`.
```{r}
example_dist <- h_get_distance(nn = c(10, 20, 30))
looks <- h_get_looks(dist = example_dist, nnE = c(10, 20, 30), nnF = c(10, 20, 30))
looks_nnrE <- looks$nnrE
...
# Example of new_RctPostProbDist when `relativeDelta = FALSE` and `decision1 = TRUE` comparing with vintage function below:
...
```{r}
# debugonce(new_ocRctPredprobDist)
set.seed(2000)
...
# Example of new_RctPostProbDist when `relativeDelta =  FALSE` and `decision1 = FALSE` comparing with vintage function below:
