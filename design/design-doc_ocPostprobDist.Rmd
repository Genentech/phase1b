...
h_get_decisionDist <- function(nnr,
  ...
  while (is.na(decision) && index_look <= length(nnr)) {
    ...
    # if ( (size_look %in% nnF && nnE) ) {
    #   qL <- 1 - postprobDist(  # Evaluating the "Stop zone" here
    #     # P(P_E < P_S + delta or P_S(1-PS)*delta | data)  >= tL this is stop
    ...
    # }
    # if (size_look %in% nnF && nnF) {
    #   qU <- postprobDist( #  Evaluating the "grey zone" here
    #     # P(P_E < P_S + delta or P_S(1-PS)*delta | data)  >= tL minus P(P_E > P_S + delta or P_S(1-PS)*delta | data) =< tU
    #     x = sum(response[1:length(response)]),
    #     n = nnE,
    #     xS = sum(response[1:length(response)]),
    #     nS = nnE,
    ...
    if (size_look %in% nnrF) {
      ...
      decision[k] <- ifelse(qL >= tL, FALSE, NA)
    }
    ...
    if (size_look %in% nnrE) {
      ...
      decision[k] <- ifelse(qU < tU, decision[k], TRUE)
    }
    all_sizes <- size_look
