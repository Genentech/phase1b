...
plotOc <- function(decision, sample_size, all_looks, wiggle_status) {
  ...
  barplot <-   
    ...
    ggplot2::xlab("look (n)") +
    # scale_fill_discrete(name = "Decision", labels = c("Go", "Stop", "Grey zone")) +
    scale_fill_manual(values = c("#009E73", "#FF0046", "lightgrey"),
                      labels = c("Go", "Stop", "Grey zone")) +
    if (wiggle_status) {
      wiggle_warning <- paste("Note that sample sizes slightly differ from the ones labed")
      barplot <- barplot + ggplot2::annotate("text", 
