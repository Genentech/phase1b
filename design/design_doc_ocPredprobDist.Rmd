...
to the two ways of calculating delta in [postprobDist()]. For the final look in [ocPredprobDist()], where a Go decision is evaluated, the margin `deltaE` is employed, and where a Stop decision is evaluated, the margin `deltaF` is employed for the final decision as illustrated here.

While the rules of [ocPredprobDist()] and [ocPredprob()] for both Decision 1 and Decision 2 are the same, the margin of difference is different for the final futility look in [ocPredprobDist()] :
