...
The rules for Stop, Go and Grey Zone (where applicable) is the same as in [ocPredprob()].
However when compared to ocPredprob, where the `response rate > p0`,
the `response rate - p0 > 0` is replaced by `response rate - p0 > delta`,
and the reason is that `p0` is now also uncertain.
...
The two ways of calculating this margin is reflected as follows in
Decision 1 and Decision 2 respectively:

# For Decision 1:----
A trial will be successful at FINAL if : (Go rule)

`Pr(P_E > p0 + deltaE | data) > tT ` for the absolute case
`Pr(P_E > (1-p0)*deltaE | data) > tT` for the relative case

A trial will be successful at INTERIM if : (Go rule)

`Pr(success at final| data) > phiU` where success is:
   - `Pr(P_E > p0 + deltaE | data) > tT ` for the absolute case
   - `Pr(P_E > (1-p0)*deltaE | data) > tT` for the relative case

A trial will be unsuccessful at FINAL if : (Stop rule)

`Pr(P_E > p0 + deltaF | data) < tF` for the absolute case
`Pr(P_E > (1-p0)*deltaF | data) < tF` for the relative case

A trial will be unsuccessful at INTERIM if : (Stop rule)

`Pr(unsuccessful at final| data) < phiL` where unsuccess is
  - `Pr(P_E > p0 + deltaF | data) > tT` for the absolute case
  - `Pr(P_E > (1-p0)*deltaF | data) > tT` for the relative case
...
`Pr(successful at final| data) > phiU` where unsuccessful is
  - `Pr(P_E > p0 + deltaE | data) > tT` for the absolute case
  - `Pr(P_E > (1-p0)*deltaE | data) > tT` for the relative case

A trial will be unsuccessful at FINAL if, note `p1` is used : (Stop rule)
...
`Pr(unsuccessful at final| data) > _phiFu_` where unsuccessful is
 - `Pr(P_E > p0 + deltaF | data) > tT` for the absolute case
 - `Pr(P_E > (1-p0)*deltaF | data) > tT` for the relative case
