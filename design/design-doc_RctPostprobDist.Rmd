...
h_get_oc_rct <- function(all_sizes, Nmax, nActive, nControl, decision) {
  ...
  data.frame(
    ExpectedN = mean(all_sizes), # but there should not be na at all
...
new_RctPostprobDist <- function(nnE,
  ...
  oc <- h_get_oc_rct(all_sizes = all_sizes, Nmax = Nmax, nActive = nActive, nControl = nControl, decision = decision)
  list(
    oc = oc,
