...
h_get_decisionDist_rct <- function(nnr,
  ...
  response <- rbinom(Nmax, size = 1, prob = ifelse(rct, pE, pS))
...
```{r}
h_get_oc_rct <- function(all_sizes, nActive, nControl, decision, nnrE, nnrF) {
...
new_RctPostProbDist <- function(nnE,
  ...
  oc <- h_get_oc_rct(all_sizes = all_sizes, nActive = nActive, nControl = nControl, decision = decision, nnrE = nnrE, nnrF = nnrF)
  list(
    oc = oc,
...
```{r}
diff_ocRctPostprobDist <- function(nn, pE, pS, deltaE, deltaF, relativeDelta = FALSE,
                                   tL = 0.8, tU = 0.8,
                                   parE = c(a = 1, b = 1),
                                   parS = c(a = 1, b = 1),
                                   randRatio = 1,
