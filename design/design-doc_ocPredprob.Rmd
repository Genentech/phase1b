...
```{r}
h_get_decision_one_predprob <- function(nnr, truep, p0, parE = c(1, 1), nnE, nnF, tT, phiU, phiL, weights) {
  index_look <- 1
  assert_numeric(nnr)
...
```{r}
h_get_decision_one_predprob(nnr = c(10, 20),
                            nnE = c(10, 20),
                            truep = 0.4,
                            nnF = c(11, 15),
                            p0 = 0.2,
                            ...
                            phiU  = 0.5,
                            phiL = 0.5)
# (nnr, p0, parE = c(1, 1), nnE, nnF, tT, phiU, phiL, weights)
```
...
```{r}
h_get_decision_two_predprob <- function(nnr, truep, p0, p1, parE = c(1, 1), nnE, nnF, tL, tU, tT, weights, qU, phiFu, phiU) {
  index_look <- 1
  assert_numeric(nnr)
  size_look <- nnr[index_look]
  all_sizes <- decision <- NA
...
ocPredprob <- function(nnE, truep, p0, p1, tT, tFu, phiL = 0.2, phiU = 0.8, parE = c(1, 1),
  ...
  for (k in seq_len(sim)) {
    ...
    if (is.na(decision[k]) && (j <= length(nnr)) && decision1 == decision1) {
      tmp <- h_get_decision_one_predprob(
        # (nnr, truep, p0, parE = c(1, 1), nnE, nnF, tT, phiU, phiL, weights)
        nnr = nnr,
        truep = truep, p0 = p0, 
        nnF = nnrF, tL = tL, phiU = phiU, phiL = phiL, weights = weights
      )
      decision[k] <- tmp$decision
      all_sizes[k] <- tmp$all_sizes
    } else {
      tmp <- h_get_decision_two_predprob(
        # nnr, truep, p0, p1, parE = c(1, 1), nnE, nnF, tL, tU, tT, weights, qU, phiFu, phiU
        nnr = nnr,
        truep = truep, p0 = p0, p1 = p1,
        parE = c(1, 1), nnE = nnrE,
        nnF = nnrF, tL = tL, tU = tU, weights = weights, qU = qU, phiFu = phiFu, phiU = phiU
      )
      decision[k] <- tmp$decision
      all_sizes[k] <- tmp$all_sizes
    }
...
# (nnE, truep, p0, p1, tT, tFu, phiL = 0.2, phiU = 0.8, parE = c(1, 1),
#                        sim = 50000, wiggle = FALSE, nnF = nnE, decision1 = TRUE)
```
