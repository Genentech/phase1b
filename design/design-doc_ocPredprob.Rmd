...
## Helper function for Decision 1a
    * Decision 1 says:
    * GO = P(truep > p0) > tT,
    ...
    * EVAL = there is no evaluate BUT there are interims.... not in this code
```{r}
h_get_decision_one_predprob <- function(nnr, truep, p0, p1, parE = c(1, 1), nnE, nnF, tL, tU, weights) {
  ...
  while (is.na(decision) && index_look <= length(nnr)) {
    ...
    if (size_look %in% nnE) {
      ...
      decision <- ifelse(qU > tT, decision, TRUE) # grandma :  ifelse(qU < tU, decision, TRUE)
    }
    # Decision 1 says:
    # GO = P(truep > p0) > tT, when the prob of good response is high enough, then we have a successful trial
    # STOP = P(truep > p0) < tT =  Unsuccessful trial if not successful 
...
## Helper function for Decision 1b
    * Decision 1 says:
...
```{r}
h_get_decision_oneb_predprob <- function(nnr, truep, p0, p1, parE = c(1, 1), nnE, nnF, tL, tU, weights) {
  index_look <- 1
  assert_numeric(nnr)
  size_look <- nnr[index_look]
  all_sizes <- decision <- NA
  ...
  if (is.na(decision) && index_look == length(nnr)) {
    ...
    if (size_look %in% nnE) { # for efficacy looks
      qU <- something()
      assert_number(qU, lower = 0, upper = 1)
      decision <- ifelse(qU > phiU, decision, TRUE) # grandma :  ifelse(qU < tU, decision, TRUE)
    } else {
      if (size_look %in% nnF) { # for futility looks
      qU <- something()
      assert_number(qU, lower = 0, upper = 1)
      decision <- ifelse(qU < phiL, decision, TRUE) # grandma :  ifelse(qU < tU, decision, TRUE)
    }
  }
  list(
    decision = decision,
...
## Helper function for Decision 2
    ...
    * EVAL = there is no evaluate
```{r}
h_get_decision_two_predprob <- function(nnr, truep, p0, p1, parE = c(1, 1), nnE, nnF, tL, tU, weights) {
  index_look <- 1
  assert_numeric(nnr)
  size_look <- nnr[index_look]
  all_sizes <- decision <- NA
  ...
  while (is.na(decision) && index_look <= length(nnr)) {
    if (size_look %in% nnE) {
      qU <- 1 - predprob(
        ...
        Nmax = nnE[length(nnE)],
        p = p1,
        thetaT = tF, # the  code says : P(truep < p1) > tF
        parE = parE
      )$result
      assert_number(qU, lower = 0, upper = 1)
...
## h_get_decision_predprob from grandma code
```{r}
