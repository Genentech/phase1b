...
h_get_decision_one_predprob <- function(nnr, truep, p0, parE = c(1, 1), nnE, nnF, tT, phiU, phiL) {
  ...
  if (is.na(decision) && index_look <= length(nnr)) { # interim
    if (size_look %in% nnE) { # GO decision for interim
      ...
      decision <- ifelse(interim_qU > tT, TRUE, FALSE) # grandma :  ifelse(qU < tU, decision, TRUE)
    }
    if (size_look %in% nnF) {
    ...
    data <- data.frame(responders = 0:sum(x = response[1:size_look]),
    ...
    if (is.na(decision) && index_look <= length(nnr)) {
      ...
      if (size_look %in% nnE) { # for efficacy looks
        # what is the response rate for a GO decision : P(response rate for GO > tT ) 
        ...
        # it is an interim GO if P(response rate for GO > tT ) > phiU
        decision <- ifelse(interim_qU > phiU, TRUE, ifelse(interim_qU > phiL, FALSE, decision))
      }
      if (size_look %in% nnF) { # for futility looks
        # what is the response rate for a STOP decision : P(response rate for STOP < tT ) 
        ...
        assert_number(interim_qU, lower = 0, upper = 1)
        # it is an interim STOP if P(response rate for STOP < tT )  < phiL
        decision <- ifelse(interim_qU < phiL, TRUE, ifelse(interim_qU > phiL, FALSE, decision))
      }
    }
    all_sizes <- size_look
...
## 2 Helper function for Decision 2
    * Decision 2 says:
    * GO = P(truep > p0) < tT =  when the prob of poor response is low, then we have a successful trial 
    * STOP = P(truep < p1) > tF:
    when the prob of poor response is high enough, then we have a UNsuccessful trial
    * interim included here .....
    * interim STOP : P (failure) > phiFu
    * interim GO : P (failure) > phiU
```{r}
h_get_decision_two_predprob <- function(nnr, truep, p0, p1, parE = c(1, 1), nnE, nnF, tT, tF, phiFu, phiU) {
  ...
  if (is.na(decision) && index_look <= length(nnr)) {
    ...
    if (size_look %in% nnE) { # for efficacy looks
      ...
      assert_number(final_qU, lower = 0, upper = 1)
      decision <- ifelse(final_qU > tT, FALSE, ifelse(interim_qU > phiU, TRUE, decision))
    }
    if (size_look %in% nnF) { # for futility looks
      final_qU <- 1 - postprob( # based on all data, the posterior probability of success is a STOP when P(truep > p0) < tT
        x = response,
        n = size_look,
        p = truep,
        parE = c(1, 1),
