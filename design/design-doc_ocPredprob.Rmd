...
## 1a Helper function for Decision 1a (does not include interim)
    * Decision 1 says:
    * GO = P(truep > p0) > tT,
...
## 1b Helper function for Decision 1b
    * Decision 1 says:
...
## 2a Helper function for Decision 2a
    * Decision 2 says:
    * STOP = P(truep < p1) > tF:
...
h_get_decision_two_predprob <- function(nnr, truep, p0, p1, parE = c(1, 1), nnE, nnF, tL, tU, weights) {
  ...
  while (is.na(decision) && index_look <= length(nnr)) {
    ...
    if (size_look %in% nnF) {
      ...
      assert_number(qU, lower = 0, upper = 1)
      decision <- ifelse(qU < tF, decision, FALSE)
    }
    all_sizes <- size_look
    index_look <- index_look + 1
    size_look <- nnr[index_look]
...
## 2b Helper function for Decision 2b
    * Decision 2 says:
    * STOP = P(truep < p1) > tF:
    when the prob of poor response is high enough, then we have a UNsuccessful trial
    * GO = P(truep > p1) < tF =  when the prob of poor response is low, then we have a successful trial
...
h_get_decision_two_predprob <- function(nnr, truep, p0, p1, parE = c(1, 1), nnE, nnF, tL, tU, weights) {
  ...
  if (is.na(decision) && index_look == length(nnr)) {
    ...
    if (size_look %in% nnE) { # for efficacy looks
      qU <- postprob(x = sum(x = response[1:size_look]), 
                     n = size_look, 
                     p = qU, 
                     parE = c(1, 1), 
                     weights, 
                     ...
                     log.p = FALSE)
      assert_number(qU, lower = 0, upper = 1)
      decision <- ifelse(qU > phiFu, FALSE, decision)
    }
    if (size_look %in% nnF) { # for futility looks
      qU <-  postprob(x = sum(x = response[1:size_look]), 
                     n = size_look, 
...
## parent code : h_get_decision_predprob 
```{r}
