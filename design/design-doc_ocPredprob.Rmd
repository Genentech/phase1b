...
h_get_decision_one_predprob <- function(nnr, truep, p0, parE = c(1, 1), nnE, nnF, tT, phiU, phiL) {
  ...
  response <- stats::rbinom(max(nnr), size = 1, truep) # in grandma, truep = p1, but p1 only used if decision2 is specified
  ...
  if (is.na(decision) && index_look <= length(nnr)) { # interim
    if (size_look %in% nnE) { # GO decision for interim
      ...
      decision <- ifelse(interim_qU > phiU, TRUE, FALSE) # grandma :  ifelse(qU < tU, decision, TRUE)
    }
    if (size_look %in% nnF) {
      ...
      decision <- ifelse(interim_qU < phiL, TRUE, FALSE) # grandma :  ifelse(qU < tU, decision, TRUE)
    }
  } else {
    ...
    if (is.na(decision) && index_look <= length(nnr)) {
      if (size_look %in% nnE) { # for efficacy looks
        # what is the response rate for a GO decision : P(truep > pO ) > tT 
      ...
      if (size_look %in% nnF) { # for futility looks
        # what is the response rate for a STOP decision : P( truep < pO ) < tT
