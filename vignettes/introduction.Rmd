...
To demonstrate the calculation of the posterior probability of a mixed
Beta Binomial distribution and the updates parameters and weight, we
...
Where the sum $w$ has to equal 1:
...
We simply to the following:

$$ \sum_{j}^{k} * \frac {w * Beta(\alpha_j + x, \beta_j +n -x )}{Beta(\alpha_j, \beta_j)} * f(\pi | \alpha_j + x + 1, \beta_i + n - x)$$

We update the weights $w$ in `R` code with $\frac {Beta(\alpha_j + x, \beta_j +n -x )}{Beta(\alpha_j, \beta_j)}$ which is represented by the `h_getBetaMixPost()` function call:
