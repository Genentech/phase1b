...
In order to incorporate prior information and evaluate posterior probabilities, updated weights and parameters are required especially where mixed Beta Binomial distribution is involved. To demonstrate the calculation the updated parameters and weights, we
illustrate the reasoning as follows.

We present the Binomial and Beta densities to begin:
...
\begin{equation}
$$f(\pi) = \sum_{j = 1}^{k} w_j \frac {1} {B(\alpha_j, \beta_j)} \pi^{\alpha_j-1} (1-\pi)^{\beta_j-x}$$
(\#eq:weightedBetaPrior)
\end{equation}

Where the sum $w$ has to equal 1: $\sum_{j = 1}^{k} w_j = 1$.

Where the unweighted version looks like:
\begin{equation}
$$f(\pi | \alpha_j, \beta_j) = \frac {1} {B(\alpha_j, \beta_j)} \pi^{\alpha_j-1} (1-\pi)^{\beta_j-x}$$
(\#eq:unweighted BetaPrior)
\end{equation}

Here, the product of likelihood and prior gives the kernal for the posterior where we exclude the normalising constant $\frac {1}{f(x)}$:

\begin{equation}
Posterior = $f(\pi | x) \infty f(\pi) * f(x | \pi) $
$$f(x|\pi) = \binom{n}{k} \pi^x (1-\pi)^{n-x}$$
(\#eq: BayesBetaBinom)
\end{equation}

\begin{equation}
$$ \infty \ \pi^x (1-\pi)^{n-x} * (\sum_{j = 1}^{k} \ w_j \frac {1}{Beta(\alpha_j, \beta_j)} * \pi^{\alpha-1}(1-\pi){^\beta-1} ) $$
(\#eq:WeightedBayes)
\end{equation}

\begin{equation}
 $$ \sum_{j = 1}^{k} \frac {w_j}{Beta(\alpha_j, \beta_j)} * \pi^{\alpha_j + x-1}(1-\pi)^{\beta_j+ n -x-1} * \frac{\beta(\alpha_j + x, \beta_j +n -x)}{\beta(\alpha_j + x, \beta_j +n - x)}$$
 (\#eq:rearrangingWeightedBayes)
 \end{equation}

From \@ref(eq:rearrangingWeightedBayes), we know that the prior is embedded and can be simplified to the following:

<!-- $ f(\pi | \alpha_j + x + 1, \beta_i + n - x) $ a Beta distribution -->
<!-- $$  f(\pi | \alpha_j + x + 1, \beta_i + n - x) = \frac{\pi^{\alpha_j + x-1}(1-\pi)^{\beta_j+ n -x-1}}{{Beta(\alpha_j + x, \beta_j +n -x )}}$$ -->

\begin{equation}
$$ \sum_{j}^{k}  \frac {w_j * Beta(\alpha_j + x, \beta_j +n -x )}{Beta(\alpha_j, \beta_j)} * f(\pi | \alpha_j + x + 1, \beta_i + n - x)$$
 (\#eq:simplifiedWeightedBayes1)
 \end{equation}

 Which can be further simplified to its kernal @\ref(eq:simplifiedWeightedBayes2):
 \begin{equation}
$$\sum_{j}^{k} w_j \frac {Beta(\alpha_j + x, \beta_j +n -x )}{Beta(\alpha_j, \beta_j)}$$
 (\#eq:simplifiedWeightedBayes2)
 \end{equation}
 The kernal of @\ref(eq:simplifiedWeightedBayes2) is incorporated in the function call as:
`exp(lbeta(alpha + x, beta + n - x) - lbeta(alpha,  beta))`.

Finally, we update the weights $w$ where by
 \begin{equation}
$$w_j = \frac {w_j * \frac {Beta(\alpha_j + x, \beta_j +n -x )}{Beta(\alpha_j, \beta_j)}} {\sum_{} (w_j * \frac {Beta(\alpha_j + x, \beta_j +n -x )}{Beta(\alpha_j, \beta_j)})} $$
 (\#eq:calc_weights)
 \end{equation}

and we update the parameters simply as:
 \begin{equation}
* updated $\alpha = \alpha + x $$
* updated $\beta = \beta + n - x $$
 (\#eq:updated_pars)
 \end{equation}

Both @\ref(eq:calc_weights) and @\ref(eq:updated_pars) are incorporated by the `h_getBetaMixPost()` function call.
...
rate of 75%. The planed interim and final looks are at 25, 40 and 80 patients
which is entered as a vector in the `nn` argument. `p` is the true PET-CR rate
we assumed, and we are targeting for a `delta` improvement over control
(`delta` ranges from 0 to 1). `tT` is the threshold for the probability to be
...
set.seed(4)
res1 <- ocPredprobDist(
  nn = c(25, 40, 80), p = 0.75, delta = 0.15, tT = 0.6, phiL = 0.2, phiU = 0.8,
  parE = c(5.75, 4.25), parS = c(75, 75), ns = 1e1
)
...
However, the user needs to increase the number to 10,000 or higher to
...
Mysim <- sapply(p_par, function(x) {
  res <- ocPredprobDist(c(25, 40, 80), x,
    delta = 0.15, tT = 0.6, phiL = 0.8, phiU = 0.2,
    parE = c(5.75, 4.25), parS = c(75, 75), ns = 1e1
  )
  return(res$oc)
})
