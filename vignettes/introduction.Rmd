...
$P_E\sim \sum_{j=1}^kw_i\text{Beta}(a_j,b_j)$. In Section, we illustrate a mixture Beta distribution and how weights and parameters are updated with data.
...
Where the sum $w_j$ has to equal 1: $\sum_{j = 1}^{k} w_j = 1$. With the normalizing constant removed, the weights will later be updated.

Where component $j$ has the Beta density:
\begin{align}
...
Then, the product of likelihood and prior $f(\pi | x) \propto f(\pi) f(x | \pi)$ gives the kernel for the posterior where we exclude the normalising constant $\frac{1}{f(x)}$ as well as any other multiplicative constants not depending on $\pi$:
...
(\#eq:WeightedBayes)
 ...
 \begin{align}
f(\pi | x) \propto \sum_{j = 1}^{k} \frac {w_j}{B(\alpha_j, \beta_j)} \pi^{\alpha_j + x -1} (1-\pi)^{\beta_j + n - x -1} \frac{B(\alpha_j + x, \beta_j + n - x)}{B(\alpha_j + x, \beta_j + n - x)}
...
\begin{align}
 f(\pi | x) \propto \sum_{j}^{k}  {w_j} \frac {B(\alpha_j + x, \beta_j +n -x )}{B(\alpha_j, \beta_j)} f(\pi | \alpha_j + x, \beta_i + n - x)
 (\#eq:simplifiedWeightedBayesone)
  \end{align}
\
Which can be further simplified to its kernel from \@ref(eq:simplifiedWeightedBayesone) :
\
\begin{align}
f(\pi | x) \propto \sum_{j}^{k} w_j \frac {B(\alpha_j + x, \beta_j +n -x )}{B(\alpha_j, \beta_j)}
 (\#eq:simplifiedWeightedBayestwo)
 \end{align}
\
The weights from \@ref(eq:simplifiedWeightedBayestwo) are updated to ensure $\sum w_j = 1$:
\
\begin{align}
\bar{w_j} = w_j \frac {\frac {B(\alpha_j + x, \beta_j +n -x )}{B(\alpha_j, \beta_j)}} {\sum_{j} w_j  \frac {Beta(\alpha_j + x, \beta_j +n -x )}{B(\alpha_j, \beta_j)}}
(\#eq:calcweights)
\end{align}
