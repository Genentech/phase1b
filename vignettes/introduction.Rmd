...
\begin{equation}
f(\pi | \alpha_j, \beta_j) = \frac {1} {B(\alpha_j, \beta_j)} \pi^{\alpha_j-1}
(\#eq:unweightedBetaPrior)
\end{equation}

Here, the product of likelihood and prior  $f(\pi | x) \propto f(\pi) f(x | \pi)$ gives the kernal for the posterior where we exclude the normalising constant $\frac {1}{f(x)}$:

\begin{equation}
 f(\pi | x) \propto \pi^x (1-\pi)^{n-x}\sum_{j = 1}^{k} \ w_j \frac {1}{Beta(\alpha_j, \beta_j)} \pi^{\alpha_j-1}(1-\pi)^{\beta_j-1}
(\#eq:WeightedBayes)
\end{equation}

\begin{equation}
f(\pi | x) \propto \sum_{j = 1}^{k} \frac {w_j}{Beta(\alpha_j, \beta_j)} \pi^{\alpha_j + x -1} (1-\pi)^{\beta_j + n - x -1} \frac{Beta(\alpha_j + x, \beta_j + n - x)}{Beta(\alpha_j + x, \beta_j + n - x)}
 (\#eq:rearrangingWeightedBayes)
 \end{equation}

From \@ref(eq:rearrangingWeightedBayes), we know that the updated Beta prior $f(\pi | \alpha + x_j, \beta_j + n -x) = \frac {\pi^{\alpha_j + x -1} (1-\pi)^{\beta_j + n - x -1}} {Beta(\alpha_j + x, \beta_j + n - x)}$ is embedded and can be simplified to the following:

\begin{equation}
 f(\pi | x) \propto \sum_{j}^{k}  {w_j} \frac {Beta(\alpha_j + x, \beta_j +n -x )}{Beta(\alpha_j, \beta_j)} f(\pi | \alpha_j + x, \beta_i + n - x)
 (\#eq:simplifiedWeightedBayesone)
 \end{equation}

 Which can be further simplified to its kernal from @\ref(eq:simplifiedWeightedBayesone) :
 \begin{equation}
f(\pi | x) \propto \sum_{j}^{k} w_j \frac {Beta(\alpha_j + x, \beta_j +n -x )}{Beta(\alpha_j, \beta_j)}
 (\#eq:simplifiedWeightedBayestwo)
 \end{equation}

The kernal in @\ref(eq:simplifiedWeightedBayestwo) is important to update the weights $w_j$:
\

\begin{equation}
\bar{w_j} = w_j \frac {\frac {Beta(\alpha_j + x, \beta_j +n -x )}{Beta(\alpha_j, \beta_j)}} {\sum_{j} w_j  \frac {Beta(\alpha_j + x, \beta_j +n -x )}{Beta(\alpha_j, \beta_j)}}
(\#eq:calcweights)
\end{equation}
...
\begin{equation}
\alpha_{updated} = \alpha + x
 (\#eq:updatedpars1)
\end{equation}

\begin{equation}
\beta_{updated} = \beta + n - x
(\#eq:updatedpars2)
\end{equation}
